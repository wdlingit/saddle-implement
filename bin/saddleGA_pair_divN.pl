...
my $usage = "Usage: saddleGA_pair_divN.pl [options] <genePairedPrimerFile> <outPrefix>\n";
$usage   .= "    Group division:\n";
$usage   .= "        -group      : if a number, divide targets into this number of groups\n";
$usage   .= "                      if a file of group name-target numbers, divide targets\n";
$usage   .= "                      into specified groups with specified numbers (default: $groupConfig)\n";
...
#
# an individual is a permutation array, as initialized in initArray
# in a permutation array, i-th element, $targetArray[$permu[$i]], is the target
#
# paritionMapping is for indicating this i-th element is in which partition
my @targetArray = sort keys %genePrimerPairs; # for quick reference
my @initArray = sort keys %genePrimerPairs;
my @partitionMapping = ();

# set partitions for numbers
if($groupConfig=~/^\d+$/){
    $nDiv = $groupConfig;
    # make the array evenly with numbers 1..groupConfig
    for(my $i=0;$i<@initArray;$i++){
        # simple increasing array to be permutated
        $initArray[$i] = $i;
        # assign group, ugly but simple
        my $mod = $i%$groupConfig;
        $partitionMapping[$i] = $mod+1;
    }
    @partitionMapping = sort {$a<=>$b} @partitionMapping;
}elsif((-e $groupConfig) && (-f $groupConfig)){
    $nDiv = 0;
    for(my $i=0;$i<@initArray;$i++){
        # simple increasing array to be permutated
        $initArray[$i] = $i;
    }
    open(FILE,"<$groupConfig");
    while(<FILE>){
        chomp;
        my @t = split;
        my @arr = ($t[0])x$t[1];
        push @partitionMapping, @arr;
        $nDiv++;
    }
    close FILE;
    
    die "targets in $groupConfig (".(@partitionMapping).") is not the same with $primerFile (".(@targetArray).")\n"
        if @partitionMapping != @targetArray;
}else{
    die "-group ($groupConfig) not a number nor a file\n";
}

# checks
die "number of groups must be greater than 1\n" if $nDiv<=1;
...
sub fitness {
    ...
    # chunks of genes    
    my %partitionArrayHash = ();
    for(my $i=0;$i<@$chromosome;$i++){
        push @{$partitionArrayHash{$partitionMapping[$i]}}, $targetArray[$$chromosome[$i]];
    }
    my @geneChunks = ();
    for my $group (sort keys %partitionArrayHash){
        push @geneChunks, \@{$partitionArrayHash{$group}};
    }
...
sub report_bestN{
    ...
    for(my $i=0;$i<$bestN;$i++){
        ...
        for(my $j=0; $j<@{$$populationRef[$sortedIdx[$i]]}; $j++){
            $groupTarget{$partitionMapping[$j]}{$targetArray[${$$populationRef[$sortedIdx[$i]]}[$j]]}=1;
        }
        # output
        for my $group (sort keys %groupTarget){
...
sub chromosome_signature {
    ...
    # in target listing order, reporting groups    
    my @ansArr;
    for(my $i=0;$i<@$chromosome;$i++){
        $ansArr[$$chromosome[$i]] = $partitionMapping[$i];
    }
    
    return "@ansArr";
}

# dummy OX crossover, without caring groups
sub order_crossover_default {
    ...
    for my $g (@$parent1){
        if(exists $selectedHash{$g}){
            push @new_ans, $ansHash1{$targetArray[$g]};
        }else{
            push @new_ans, $ansHash2{$targetArray[$g]};
        }
    }
...
# modified OX crossover, copy a region from parent1 and try preserving groupship from parent2
sub order_crossover {
    my ($parent1, $parent2, $ans1, $ans2) = @_;
    my @child = @$parent1;
    my @new_ans = ();
    ...
    ### Fill remaining positions by trying to keep original groups from parent2
    my %filled = ();
    my %unfilledIndexHash = ();
    for(my $i=0;$i<@child;$i++){
        if($start<=$i && $i<=$end){
            # selected region from parent1
            $child[$i] = $$parent1[$i];
            $filled{$child[$i]} = 1;
        }elsif(not exists $selectedHash{$$parent2[$i]}){
            # non-selected region and not a selected number
            $child[$i] = $$parent2[$i];
            $filled{$child[$i]} = 1;
        }else{
            # otherwise, set -1 to be filled by unused numbers
            $child[$i] = -1;
            $unfilledIndexHash{$i} = 1;
        }
    }
    
    # get unfilled numbers
    my %unfilled = (); # number -> original group
    my @unfilledArray = ();
    for(my $i=0;$i<@$parent2;$i++){
        if(not exists $filled{$$parent2[$i]}){
            $unfilled{$$parent2[$i]} = $partitionMapping[$i];
            push @unfilledArray, $$parent2[$i];
        }
    }
    # get group-unfilled slots, ordered
    my %groupSlots = (); # group-slots
    for(my $i=0;$i<@child;$i++){
        if($child[$i]==-1){
            push @{$groupSlots{$partitionMapping[$i]}}, $i;
        }
    }
    # fill numbers if an empty slot with original group
    for(my $i=0;$i<@unfilledArray;$i++){
        my $desiredGroup = $unfilled{$unfilledArray[$i]};
        if((exists $groupSlots{$desiredGroup}) && @{$groupSlots{$desiredGroup}}>0){
            my $slot = shift @{$groupSlots{$desiredGroup}};
            $child[$slot] = $unfilledArray[$i];
            delete $unfilled{$unfilledArray[$i]};
            delete $unfilledIndexHash{$slot};
        }
    }
    # fill rest numbers to unfilled slots
    if( (keys %unfilledIndexHash) != (keys %unfilled) ){
        # one minor check, should not happen
        my $n1 = (keys %unfilledIndexHash);
        my $n2 = (keys %unfilled);
        die "UNFILLED not matching: $n1 $n2\n";
    }
    my @unfilledIndexes = sort {$a<=>$b} keys %unfilledIndexHash;
    for(my $i=0;$i<@unfilledArray;$i++){
        next if not exists $unfilled{$unfilledArray[$i]};
        my $slot = shift @unfilledIndexes;
        $child[$slot] = $unfilledArray[$i];
    }
    # minor check, only for testing
    my $inTEST = 1;
    if($inTEST){
        my %numbers = ();
        for(my $i=0;$i<@child;$i++){
        ...
        die "FILL MISMATCH @child\n" if (keys %numbers)!=@child;
    }
...
sub swap_mutation {
    ...
    # ensure the two indices in different group
    while($partitionMapping[$i] eq $partitionMapping[$j]){
        $i = int(rand(@$chromosome));
        $j = int(rand(@$chromosome));
    }
